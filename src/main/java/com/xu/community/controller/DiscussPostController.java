    ...
    public String getDiscussPost(@PathVariable("discussPostId") int discussPostId, Model model, Page page) {
        ...
        if (commentList != null) {
            for (Comment comment : commentList) {
                ...
                //点赞
                likeCount = likeService.findEntityLikeCount(ENTITY_TYPE_COMMIT, comment.getId());
                commentVo.put("likeCount", likeCount);
                //点赞状态
                likeStatus = hostHolder.getUser() == null ? 0 : likeService.findEntityLikeStatus(hostHolder.getUser().getId(), ENTITY_TYPE_COMMIT, comment.getId());
                commentVo.put("likeStatus", likeStatus);

                //回复列表
                List<Comment> replyList = commentService.findCommentsByEntity(ENTITY_TYPE_COMMIT, comment.getId(), 0, Integer.MAX_VALUE);
                //回复VO列表
                ...
                if (replyList != null) {
                    for (Comment reply : replyList) {
                        ...
                        //点赞
                        likeCount = likeService.findEntityLikeCount(ENTITY_TYPE_COMMIT, reply.getId());
                        replyVo.put("likeCount", likeCount);
                        //点赞状态
                        likeStatus = hostHolder.getUser() == null ? 0 : likeService.findEntityLikeStatus(hostHolder.getUser().getId(), ENTITY_TYPE_COMMIT, reply.getId());
